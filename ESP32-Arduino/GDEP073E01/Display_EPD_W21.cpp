...
void PIC_PARTIAL_display_begin() {
    EPD_W21_WriteCMD(0x10);  // Start display update
}

void PIC_PARTIAL_display_line(const unsigned char* lineData, int lineNumber) {
    if (lineNumber < 0 || lineNumber >= 480) return;  // Boundary check
...
void PIC_PARTIAL_display_end() {
    EPD_W21_WriteCMD(0x12);   // Display refresh
    EPD_W21_WriteDATA(0x00);
    delay(1);   // Minimum 200uS delay required
    lcd_chkstatus();          // Wait for idle
}
